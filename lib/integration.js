...
function mockResponder(integration, properties) {
  return (req, res) => {
    const response = integration.responses.default;
    const template = response.responseTemplates
      ? response.responseTemplates['application/json']
      : response.responseTemplate;
    respond(req, res, response, template, properties);
  };
}
