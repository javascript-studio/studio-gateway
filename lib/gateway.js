  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
        ...
        try {
          method.parameters.forEach((param) => {
            if (param.in === 'header') {
              ...
              throw new Error(`Unknown parameter.in "${param.in}" for `
                + `${req.method} ${u.pathname}`);
            }
          });
        } catch (e) {
      ...
      if (!request_templates) {
        fail(res, `Missing "requestTemplates" for ${req.method} ${u.pathname}`);
        return;
      }
      const request_template = request_templates['application/json'];
      if (!request_templates) {
        fail(res, 'Missing "application/json" in requestTemplates for '
          + `${req.method} ${u.pathname}`);
        return;
      }
      const context = {};
      ...
      try {
        ...
        fail(res,
          `Error in request template for ${req.method} ${u.pathname}: ${e}`);
        return;
      }
